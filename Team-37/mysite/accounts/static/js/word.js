...
class Board {
    ...
    constructor() {
        ...
        this.genWordList = ["iron", "food", "mineral", "folate", "human", "carbs", "zinc", "protein", "omega3", "fiber", "calcium", "fats", "cipher", "niacin", "vitamin"];
    ...
    mouseReleased() {
        ...
        if (this.genWordList.includes(completedWord)) {
            this.wordList.forEach((letter) => {
                ...
                // let alertMessage = "Foods containing " + nutrient + ": " + foods.join(", ");

            });
            let foodWord = getFoodsForNutrient(completedWord);
            let alertMessage = completedWord + " is found in " + foodWord;
            alert(alertMessage);
        }
...
function getFoodsContainingNutrient(nutrient) {
    // Your logic to fetch the list of foods containing the nutrient goes here
    // Replace the example logic below with your actual implementation

    // Example logic: A predefined mapping of nutrients to foods
    const nutrientFoods = {
        vitaminA: ['Carrots', 'Sweet potatoes', 'Spinach', 'Kale'],
        vitaminB: ['Salmon', 'Tuna', 'Leafy greens', 'Eggs'],
        vitaminC: ['Oranges', 'Strawberries', 'Bell peppers', 'Broccoli'],
        // Add more mappings for other nutrients as needed
    };

    return nutrientFoods[nutrient] || [];
}
...
function getFoodsForNutrient(nutrient) {
    const nutrientFoods = {
        iron: ['Spinach', 'Red meat', 'Lentils', 'Quinoa'],
        food: ['Apple', 'Chicken', 'Rice', 'Salmon'],
        mineral: ['Kale', 'Almonds', 'Beans', 'Seeds'],
        folate: ['Avocado', 'Asparagus', 'Beans', 'Broccoli'],
        human: ['Milk', 'Yogurt', 'Cheese', 'Eggs'],
        carbs: ['Bread', 'Pasta', 'Rice', 'Potatoes'],
        zinc: ['Pumpkin seeds', 'Beef', 'Oysters', 'Cashews'],
        protein: ['Chicken breast', 'Fish', 'Tofu', 'Greek yogurt'],
        omega3: ['Salmon', 'Sardines', 'Chia seeds', 'Walnuts'],
        fiber: ['Whole grains', 'Legumes', 'Berries', 'Nuts'],
        calcium: ['Milk', 'Cheese', 'Yogurt', 'Leafy greens'],
        fats: ['Avocado', 'Olive oil', 'Coconut oil', 'Nuts'],
        cipher: ['Cabbage', 'Kohlrabi', 'Radish', 'Turnip'],
        niacin: ['Chicken', 'Tuna', 'Mushrooms', 'Peanuts'],
        vitamin: ['Oranges', 'Berries', 'Leafy greens', 'Citrus fruits']
        // Add more mappings for other nutrients as needed
    };

    const foods = nutrientFoods[nutrient.toLowerCase()] || [];
    return foods;
}
